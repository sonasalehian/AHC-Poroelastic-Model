...
def equation_parameters(parameters, domain, mt):
    ...
    # Subdomain
    K = fem.TensorFunctionSpace(domain, ("DG", 0))
...
def solve(parameters):
    ...
    # Check whether the specified output_dir exists or not
    isExist = os.path.exists(parameters['output_dir'])
    if not isExist:
        # Create the directory
    ...
    with io.XDMFFile(domain.comm, f"{parameters['output_dir']}/facet_tags.xdmf", "w") as xdmf:
        ...
        xdmf.write_meshtags(
            mt, geometry_xpath=f"/Xdmf/Domain/Grid[@Name='{domain.name}']/Geometry")
        xdmf.write_meshtags(
            ft, geometry_xpath=f"/Xdmf/Domain/Grid[@Name='{domain.name}']/Geometry")
